...
function fibo() {
  ...
  const praragraph = document.createElement("li");
...
function calcularFactorial(){
  const facNumber = parseInt(document.getElementById("inputFacto").value);

  console.log(facNumber);
  let result = facNumber;
  for(i=facNumber -1; i>=1; i--){
      result = result * i
  }

  console.log(result);
  const ul = document.getElementById('factoContainer')
...
function calcularFibo(){
  ...
  fibonacci(document, "#fiboContainer",fiboNumber);
